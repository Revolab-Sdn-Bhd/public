...
class Knowledge:
    def __init__(
        ...
        *,
        ...
        if self.module == "pre_login":
            ...
            self.overall_collec = "pre_v2"  # a fallback collection/table name if needed
        elif self.module == "post_login":
            self.faq_collection_name = POST_LOGIN_COLLECTION  # e.g. "post_login_faqs"
            self.overall_collec = "post_v2"
    ...
    def upload_excel_faq_to_vdb(self, excel_file: str, collection_name: str) -> None:
        ...
        self.upload_excel_fee_comm_faq_to_vd(
            excel_file, collection_name, file_id, filename
        )

    def upload_excel_fee_comm_faq_to_vd(
        self, excel_file: str, collection_name: str, file_id: str, filename: str
    ) -> None:
        """
        ...
        for sheet_name, df in sheets_dict.items():
            ...
            required_cols = {"Item", "Type", "Fees/Commissions JOD"}
            if not required_cols.issubset(set(df.columns)):
                logging.warning(
                    f"Sheet '{sheet_name}' is missing one or more required columns. Found columns: {df.columns.tolist()}"
                )
    ...
    def upload_product_notes_v2_to_vdb(
        ...
        print(f"Points uploaded: {points}")
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["faq", "conversation", "product", "product_v2"],
        required=True,
    )
    parser.add_argument(
